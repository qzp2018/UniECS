...
def get_loss(model, images1, texts1, images2, texts2, tags, item_ids1, item_ids2, loss_fn, args):
    ...
    sim_v2m = torch.matmul(fused1, visual_only2.t()) * logit_scale  # 视觉-多模态
    ...
    v2f_loss = contrastive_loss(fused1, visual_only2)
        ...
        if args.accum_freq == 1:
            ...
            if args.world_size > 1:
                ...
                with autocast(enabled=(args.precision == "amp")):
                    ...
                    global_sim_v2m = torch.matmul(global_fused1, global_visual_only2.t()) * latest_logit_scale
                    ...
                    global_v2f_loss = contrastive_loss(global_fused1, global_visual_only2)
                ...
                with autocast(enabled=(args.precision == "amp")):
                    ...
                    large_sim_v2m = torch.matmul(large_fused1, large_visual_only2.t()) * latest_logit_scale
                    ...
                    large_v2f_loss = contrastive_loss(large_fused1, large_visual_only2)
...
def evaluate(model, data, epoch, args, steps):
    ...
    with torch.no_grad():
        for i in range(dataloader.num_batches):
            ...
            # sim_v2m = torch.matmul(fused1, visual_only2.t()) * logit_scale
